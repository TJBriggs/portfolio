<?php
    ...
    if($group_count <= 3){
        for($i = 0; $i < $group_count; $i++) {
            ...
            if(($thirds_content[$i]['thirds_video'] == '')){
                $emptyness = 'empty';
            }
...
<?php if(get_sub_field('section_bg_color')): ?>
    ...
    <article class="page__group thirds<?php echo $classes; ?> <?php echo $section_bg; ?>">
<?php endif; ?>
        ...
        <?php endif; ?>
            <?php if(count($content_section) <= 3) : $i = 0; //if normal 1-3 groups of content ?>
                <?php while(has_sub_field('thirds_content')) : $i++; ?>
                    <?php  $video = get_sub_field('thirds_video'); ?>
                    <?php  $caption = get_sub_field('thirds_caption'); ?>
                    <?php if(get_sub_field('thirds_video') != '') : ?>
                        <?php
                            $empty_class = '';
                            if($i == 2) {
                        ...
                        <ul class="slides">
                            <?php while(has_sub_field('thirds_content')) : ?>
                                <li class="thirds__slide-item">
                                    <?php the_sub_field('thirds_video'); ?>
                                </li>
                            <?php endwhile; ?>
                        </ul> <!-- // slides -->
                    </div> <!-- // flexslider -->
